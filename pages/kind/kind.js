...
Page({
    ...
    //去结算
    enterSubmit() {
        //10ms后才检查，因为输入框onblur后，要把东西添加入购物车，要等一下
        setTimeout(() => {
            if (this.data.bubble > 0 || app.globalData.globalCar.length > 0) {
                wx.navigateTo({
    ...
    inputChange(e) {
        let currentNum = e.detail.value - 0;
        let price = e.currentTarget.dataset.price
        let goodsId = e.currentTarget.id;
        let isFind = false;
        for (let index in app.globalData.globalCar) {
